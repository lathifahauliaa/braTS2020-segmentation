"""
Training script — Plain 3D U-Net on BraTS 2020 (baseline).

Digunakan sebagai pembanding terhadap Attention 3D U-Net (train.py).
Split data, hyperparameter, dan scheduler identik dengan train.py
...
from model_unet3d import UNet3D, count_parameters
...
if IS_KAGGLE:
    ...
    CHECKPOINT_DIR = '/kaggle/working/checkpoints_unet3d'
elif IS_COLAB:
    ...
    CHECKPOINT_DIR   = f'{COLAB_DRIVE_ROOT}/checkpoints_unet3d'
else:
    DATA_ROOT      = r'D:\skripsi\dataset\brats\archive\BraTS2020_TrainingData\MICCAI_BraTS2020_TrainingData'
    CHECKPOINT_DIR = r'D:\skripsi\checkpoints_unet3d'

VIZ_DIR  = os.path.join(CHECKPOINT_DIR, 'viz_unet3d')
LOG_PATH = os.path.join(CHECKPOINT_DIR, 'train_unet3d.log')
...
def _plot_history(history, viz_dir):
    ...
    fig.suptitle('Training History — Plain 3D U-Net', fontsize=13, fontweight='bold')
...
def _plot_val_predictions(model, val_loader, viz_dir, n_patients=N_VIZ_PATIENTS):
    ...
    for idx, (images, masks) in enumerate(val_loader):
        ...
        img_np = images.squeeze(0).cpu().numpy()
...
def main():
    ...
    try:
        print("=" * 72)
        print("  BraTS 2020 — Plain 3D U-Net Training (Baseline)")
        print("=" * 72)
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            if mean_dice > best_mean_dice:
                ...
                no_improve     = 0
                ckpt_path      = os.path.join(CHECKPOINT_DIR, 'best_model_unet3d.pth')
                torch.save({
                    'epoch':                epoch,
        ...
        print(f"Checkpoint: {os.path.join(CHECKPOINT_DIR, 'best_model_unet3d.pth')}")

        # ── Final evaluation on test set using best checkpoint ─────────────────
        print("\n" + "=" * 72)
        print("  Final Evaluation on Test Set")
        print("=" * 72)
        ckpt = torch.load(os.path.join(CHECKPOINT_DIR, 'best_model_unet3d.pth'),
                          map_location=DEVICE)
